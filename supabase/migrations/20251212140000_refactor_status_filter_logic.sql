-- Migration: Refactorise la logique de filtrage par statut
-- Date: 2025-12-12
-- Description: Cette migration refactorise la fonction get_investor_list_details pour rendre la logique
--              de filtrage par statut plus robuste. Le statut de l'utilisateur ('New', 'Active', 'Inactive')
--              est maintenant calculé explicitement dans une CTE avant d'appliquer le filtre,
--              plutôt que d'être calculé au sein même de la clause WHERE finale.
...
BEGIN
  ...
  filtered_profiles AS (
    ...
    WHERE
      ...
      AND (p_status_filter IS NULL OR p_status_filter = 'all' OR calculated_status = p_status_filter)
  )
  SELECT 
