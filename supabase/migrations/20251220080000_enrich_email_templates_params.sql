-- Migration: Update RPC functions to use enriched email templates with new parameters
-- This migration updates request_deposit, user_withdraw, create_contract, and calculate_monthly_profits
-- to pass new required parameters to email templates

-- ============================================================================
-- 1. UPDATE request_deposit() - Add email notification with enriched parameters
-- ============================================================================
  ...
  IF v_admin_emails IS NOT NULL THEN
    INSERT INTO public.notifications_queue (
      ...
      jsonb_build_object(
        ...
        'amount', deposit_amount,
        'email', v_user_email,
        'userName', v_user_name,                    -- ✅ NEW
        'transactionId', v_transaction_id,          -- ✅ NEW
        'paymentMethod', deposit_method,            -- ✅ NEW
        'proofUrl', p_proof_url                     -- ✅ NEW
      ),
      'high'
    );
  END IF;
...
-- ============================================================================
-- 2. UPDATE user_withdraw() - Add enriched email notification for admin
-- ============================================================================
-- Note: This requires finding the most recent user_withdraw migration
-- For now, we'll create a trigger-based solution

-- ============================================================================
-- 3. CREATE new_contract_admin notification function
-- ============================================================================
-- This will be called from create_contract
...
-- ============================================================================
-- 2. NOTE: user_withdraw() 
-- ============================================================================
-- La fonction user_withdraw est complexe et a plusieurs versions.
-- Elle envoie déjà new_withdrawal_request aux admins.
-- RECOMMENDATION: Créer une migration séparée pour cette fonction
-- car elle nécessite une analyse approfondie des paramètres existants.
-- Les nouveaux paramètres requis sont:
-- - userName (à récupérer depuis profiles)
-- - transactionId (déjà disponible)
-- - withdrawalMethod (déjà disponible dans la fonction)
-- - recipientName (à récupérer depuis les champs de retrait)

-- ============================================================================
-- 3. HELPER: Function to notify admins of new contracts
-- ============================================================================
-- Cette fonction helper sera appelée depuis create_contract
...
-- ============================================================================
-- 4. NOTE: Integration with create_contract()
-- ============================================================================
-- Pour intégrer cette notification dans create_contract, ajouter à la fin de la fonction:
/*
...
-- ============================================================================
-- 5. NOTE: calculate_monthly_profits() - Contract End Enhancement
-- ============================================================================
-- La fonction calculate_monthly_profits gère la fin des contrats.
-- Elle doit être modifiée pour calculer et passer totalProfits au template contract_ended.
-- 
-- Modification requise dans la section où le contrat se termine:
/*
  ...
  INSERT INTO public.notifications_queue (
    ...
    jsonb_build_object(
      'to', v_user_email,
      'name', v_user_name,
      'contractId', v_contract_id::TEXT,
      'amount', v_initial_amount,
