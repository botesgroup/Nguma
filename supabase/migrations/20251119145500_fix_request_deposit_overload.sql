-- Fix request_deposit function overload conflict
-- Drop the old version and ensure only the new version with p_proof_url exists

-- Drop all existing versions of request_deposit to avoid ambiguity
DROP FUNCTION IF EXISTS public.request_deposit(numeric, text);
DROP FUNCTION IF EXISTS public.request_deposit(numeric, text, text, text);
DROP FUNCTION IF EXISTS public.request_deposit(numeric, text, text, text, text);

-- Recreate the function with all parameters including p_proof_url
-- This is the final, complete version that should be used
