-- Migration: Hotfix final pour la fonction get_investor_list_details
-- Date: 2025-12-12
-- Description: Cette migration corrige le bug 'column p.banned_until does not exist'.
--              Elle ajoute une jointure avec la table 'auth.users' pour récupérer
--              la colonne 'banned_until' au bon endroit.
...
BEGIN
  ...
  WITH filtered_profiles AS (
    SELECT 
      ...
      p.country,
      p.city,
      u.banned_until, -- CORRECTION: Récupéré depuis auth.users
      p.created_at,
      ci.contracts,
      ci.total_invested
    FROM public.profiles p
    LEFT JOIN auth.users u ON p.id = u.id -- AJOUT: Jointure avec auth.users
    LEFT JOIN (
      SELECT 
        user_id, 
