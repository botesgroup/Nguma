-- Migration to secure the request_deposit function by adding server-side validation.
-- This prevents users from bypassing client-side checks.
-- This migration explicitly drops the old function signature before creating the new one
-- to avoid issues with CREATE OR REPLACE FUNCTION when parameters change.

-- Drop the old function definition (2 parameters)
DROP FUNCTION IF EXISTS public.request_deposit(NUMERIC, TEXT);

-- Drop the extended function definition (5 parameters) if it already exists from a previous failed attempt or other migration
DROP FUNCTION IF EXISTS public.request_deposit(NUMERIC, TEXT, TEXT, TEXT, TEXT);

-- Create the new function definition with all parameters and security logic
