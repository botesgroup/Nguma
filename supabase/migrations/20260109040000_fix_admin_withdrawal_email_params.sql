-- This migration provides the final fix for the admin withdrawal notification email
-- by adding all required fields to the notification_params payload.
...
BEGIN
    ...
    v_recipient_name := p_payment_details->>'recipient_name'; -- Extract recipient name
    ...
    -- Enqueue email to admins with ALL required fields
    FOR admin_record IN
...
COMMENT ON FUNCTION public.user_withdraw(numeric, text, jsonb) IS 'Processes a withdrawal and correctly saves all required data for admin email notifications.';
