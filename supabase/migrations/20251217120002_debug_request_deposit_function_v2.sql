-- Migration for further debugging the request_deposit function.
-- Modifies the RAISE EXCEPTION message to include the value of v_deposit_enabled.
...
BEGIN
  ...
  -- DEBUG: Include the value of v_deposit_enabled in the exception message
  IF NOT v_deposit_enabled THEN
    RAISE EXCEPTION 'Les dépôts sont actuellement désactivés par l''administrateur. (DEBUG: v_deposit_enabled = %)', v_deposit_enabled;
  END IF;

  -- 3. Check if the current time is within the deposit period
  IF now() NOT BETWEEN v_period_start AND v_period_end THEN
    RAISE EXCEPTION 'La période de dépôt est actuellement fermée. (DEBUG: v_deposit_enabled = %, now = %, start = %, end = %)', v_deposit_enabled, now(), v_period_start, v_period_end;
  END IF;
  ...
  IF v_deposit_count >= v_max_deposits_per_period THEN
    RAISE EXCEPTION 'Vous avez atteint la limite de % dépôts pour la période actuelle. (DEBUG: v_deposit_count = %, max_deposits = %)', v_max_deposits_per_period, v_deposit_count, v_max_deposits_per_period;
  END IF;
