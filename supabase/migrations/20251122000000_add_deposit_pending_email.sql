...
BEGIN
    ...
    -- ✅ NEW: Send deposit_pending confirmation email to the user
    ...
    -- Loop through all admins and send them an email notification
    FOR admin_record IN
        SELECT u.email FROM auth.users u
        JOIN public.user_roles ur ON u.id = ur.user_id
        WHERE ur.role = 'admin'
    LOOP
        -- Construct the payload for the admin email
        payload := jsonb_build_object(
