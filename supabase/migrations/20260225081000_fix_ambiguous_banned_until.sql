...
-- Migration: Fix Ambiguous Banned Until Column in Admin Functions
-- Date: 2026-02-25
-- Description: Resolves 'column reference banned_until is ambiguous' by specifying table aliases.

-- 1. Redéfinir get_investor_list_details pour lever l'ambiguïté
...
BEGIN
  ...
  WITH profiles_with_status AS (
    SELECT p.id, p.first_name, p.last_name, p.post_nom, p.email, p.phone, p.country, p.city, p.created_at,
           u.banned_until as auth_banned_until, -- Utilise celui de auth.users comme référence
           w.total_balance, 
           ci.total_invested, 
           ci.contracts,
           CASE 
  ...
  FROM (
    SELECT fp.id, fp.first_name, fp.last_name, fp.post_nom, fp.email, fp.phone, fp.country, fp.city,
           fp.auth_banned_until as banned_until, fp.created_at,
...
-- 2. S'assurer que les permissions sont maintenues
GRANT EXECUTE ON FUNCTION public.get_investor_list_details TO authenticated;
