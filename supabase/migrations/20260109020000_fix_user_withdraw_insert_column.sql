-- This migration provides the final fix for the user_withdraw function,
-- correcting the INSERT statement by removing the non-existent 'payment_details' column.
...
BEGIN
    ...
    -- Build a readable description from JSON details for the transaction
    ...
    -- Create transaction (Corrected INSERT statement)
    ...
    VALUES (
        ...
        'pending', 
        withdraw_method,
        v_description -- Store readable details here
...
EXCEPTION 
    WHEN OTHERS THEN
        -- Safe rollback if any error occurs
        UPDATE public.wallets
        SET 
            profit_balance = profit_balance + withdraw_amount,
            locked_balance = locked_balance - withdraw_amount
...
COMMENT ON FUNCTION public.user_withdraw(numeric, text, jsonb) IS 'Processes a withdrawal with atomic balance check, safe exception handler, and correct INSERT statement.';
